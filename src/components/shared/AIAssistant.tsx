  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-24 right-4 md:bottom-8 md:right-8 z-50 w-14 h-14 rounded-full bg-primary text-primary-foreground shadow-2xl flex items-center justify-center hover:opacity-90 transition-all duration-200 hover:scale-105"
      ...
      {open && (
        <div
          className="fixed bottom-[8.5rem] right-4 md:bottom-28 md:right-8 z-50 w-[calc(100vw-2rem)] sm:w-96 bg-card border border-border rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.2)] overflow-hidden flex flex-col"
          style={{ maxHeight: "min(600px, calc(100vh - 160px))" }}
          role="dialog"
          aria-label="AI financial assistant"
        >
          <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border bg-card/80 backdrop-blur-md">
            <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center">
              <Sparkles className="w-4 h-4 text-primary" />
            </div>
            <div>
              <div className="text-sm font-semibold text-foreground">Finance Assistant</div>
              <div className="text-xs text-muted-foreground">Ask me anything about finance</div>
            </div>
          </div>

          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-secondary/10" style={{ minHeight: 300 }}>
            {messages.map((msg, i) => (
              <div key={i} className={`flex items-start gap-2.5 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                ...
                <div className={`rounded-xl px-4 py-2.5 max-w-[85%] text-sm leading-relaxed whitespace-pre-wrap shadow-sm ${
                  msg.role === "user"
